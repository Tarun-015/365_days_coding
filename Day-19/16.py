...
def list_mutating_operations(items:list, item1, item2):
    ...
    items[:] = items[1::2]  # Keep only items at odd indices
...
def list_non_mutating_operations(items:list, item1, item2):
    ...
    new_insert = items[:3] + [item2] + items[3:]
    print("insert:", new_insert)
    ...
    remove_index = items.index(item2) if item2 in items else None
    print("remove:", items[:remove_index] + items[remove_index + 1:] if remove_index is not None else items)

    # print a list with the fourth element of items changed to None
    modify_index = items[:3] + [None] + items[4:]
    print("modify_index:", modify_index)

    # print a list with the even indices changed to None
    modify_slice = items[:]
    modify_slice[::2] = [None] * (len(modify_slice) // 2 + len(modify_slice) % 2)
    print("modify_slice:", modify_slice)

    # print a list with the even indices removed
    delete_slice = items[1::2]  # Keep only items at odd indices
...
def do_set_operation(set1, set2, set3, item1, item2):
    ...
    set1.discard(item2)  # Use discard to avoid KeyError if item2 is not present
    print(sorted(set1))
    ...
    all_unique = set1.union(set2).union(set3)
    print(sorted(all_unique))

    # print all unique elements that are in set2 but not in set3 as a sorted list
    unique_in_set2 = set2.difference(set3)
    print(sorted(unique_in_set2))

    # print all the non-common elements from both set2 and set3
    non_common = set2.symmetric_difference(set3)
    print(sorted(non_common))
